...
class Tag(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            self.slug = slugify_new(
                self.name, 4
            )  # Replace with actual slug generation logic
...
class Page(models.Model):
    ...
    is_published = models.BooleanField(
        default=False,
        help_text="This field must be checkerd for the page to be displayed.",                              
                                       )
    content = models.TextField()

    def save(self, *args, **kwargs):
        if not self.slug:
            self.slug = slugify_new(self.title, 4)  # Replace with actual slug generation logic
        super().save(*args, **kwargs)
...
class PostManager(models.Manager):
    def get_published(self):
       return self.get_queryset().filter(is_published=True).order_by('-pk')
